using ShuffleText.ShffleTextEnum;
...
{
    [VideoEffect("シャッフルテキスト", ["アニメーション"], ["Shuffle Text","シャッフルテキスト", "Shuffle Letters"], isAviUtlSupported: false)]
    internal class ShuffleTextEffect : VideoEffectBase
    {
        public override string Label => "シャッフルテキスト";

        [Display(GroupName = "シャッフルテキスト", Name = "間隔", Description = "アニメーションテキストが変化する間隔")]
        [AnimationSlider("F2", "秒", 0, 0.25)]
        public Animation Interval { get; } = new Animation(0, 0, 1000.00);

        [Display(GroupName = "シャッフルテキスト", Name = "ずれ", Description = "アニメーションテキストの文字ごとのずれ")]
        [ToggleSlider]
